...
public class Peg extends Ellipse
{
  ...
  {
    //Get Current row and store it in an array
    Peg myBoardPegs[] = app.getCurRow();
    
    setColor(origColor);
    
    //If the two pegs intersect, create a new peg and store it in the board, 
    //then snap dragged peg back to the original location
    for(int i = 0; i < 4; i++)
    {
