...
{		
	...
	root = (&(*((IP_Thread_Info *)params) -> master_counters));
	...
	if(our_send_message(fd, REGULAR_EXPR, strlen(regex), regex) != 0)
	{
	...
	if(our_send_message(fd, REMOTE_FILE_NAME, strlen(remote_name), 
		remote_name) != 0)
	{
	...
	/* loop on recv */
	len = BUFSIZE;
	if((n = our_recv_message(fd, &type, &len, message)) < 0)
	{
	...
	else
	{
		...
		{
			fprintf(stderr, "Im getting a counters table!!!!!!!\n\n\n");
			Frep_Info * counters = (Frep_Info *)message;
	...
	char ** remote_name)
{
	...
	if(colon_index == 0 || slash_index == 0)
		return -1;
    ...
    {			
		...
		else
		{
			...
			{				
				...
				{
					fprintf(stderr, "Remote string '%s' is incorrectly formatted\n", 
						argv[optind]);
					continue;	
				}	
				pthread_mutex_lock(&mu);
				
				/* add dummy elem to stack */
				Elem * new_elem = newElem(node, 0);
