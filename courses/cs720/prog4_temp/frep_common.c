...
int * next_list, Frep_Info * counters, int options[])
{   
    char * input_line;
    ...
    while (fgets(input_line, line_length+1, fp) != NULL)
    {
        ...
        if (match_regex(input_line, firmware, cur_list, next_list, counters, options))
        {
			...
			{
                print_match(fp, input_line, line_number, fname, flags);
				match_count++;
			}
	...
	/* print the summary */
    if (summary)
        print_summary(fp, fname, summary_count, flags);
...
void 
print_match(FILE * fp, char * input_line, int line_number, char * fname, int flags)
{
    char * realpath;
    pthread_mutex_lock(&mu);
    if (fp == stdin)
...
void 
print_summary(FILE * fp, char * fname, int summary_count, int flags)
{
    pthread_mutex_lock(&mu);	
    if (summary_count != 0)
    {
...
process_dir(void * params)
{
	...
	root = (&(*((Dir_Info *)params) -> root));
	fd = ((Dir_Info *)params) -> fd;
		
	if ((dp = opendir(((Dir_Info *)params) -> full_dir_path)) == NULL)
	{
		log_error(((Dir_Info *)params) -> full_dir_path, &thr_local_counters, flags);
	...
	for (i = 1; ; i++)
	{
		...
		if (lstat(full_dir_entry_path, &statbuf) < 0)
			log_error(full_dir_entry_path, &thr_local_counters, flags);
		else
		{
			...
			{	
				...
				if (S_ISREG(statbuf.st_mode))  /* regular file */
					process_file(((Dir_Info *)params) -> firmware, full_dir_entry_path, cur_list, 
						next_list, 0, &thr_local_counters, ((Dir_Info *)params) -> options);
					...
					if(cur_depth != max_depth)
					{
						...
						if (find(((Dir_Info *)params) -> cur_elem, realpath) != NULL)
							process_loop(&thr_local_counters, ((Dir_Info *)params) -> cur_elem,
									full_dir_entry_path, realpath, cur_depth, flags);
						else
						{							
						    pthread_mutex_lock(&mu);
						...
						else
						{
							if(S_ISREG(linkstatbuf.st_mode))
								process_file(((Dir_Info *)params) -> firmware, full_dir_entry_path, 
									cur_list, next_list, 0, &thr_local_counters, ((Dir_Info *)params) -> options);
							else
							{	
...
process_loop(Frep_Info * counters, Elem * cur_elem, char * full_path, 
	 char * realpath, int cur_depth, int flags)
{
...
process_file(struct machine * firmware, char * full_path, int * cur_list, 
	int * next_list, int cmdline, Frep_Info * counters, int options[])
{
	FILE * fp;
	int flags = options[0];
...
void 
log_error(char * full_path, Frep_Info * counters, int flags)
{
	if((flags & QUIET) != QUIET)
	{
		/*if(fd != 1)
		{
			...
			error_msg = strcat(full_path, strerror(errno));
			if(our_send_message(fd, ERROR_LINE, 
				strlen(error_msg), error_msg) != 0)
			{
