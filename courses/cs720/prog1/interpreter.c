...
*/
int in_set(unsigned char * brset, char c, int first_printable)
{
 ...
 */
int add_action(int * list, int action)
{
    int index;
    int i;
    i = 0;
...
int* cur_list, int* next_list)
{
    ...
    while (cur_list[address] != 0)
    {
        ...
        case 6:       /* match */
                if((flags & DEBUG) == DEBUG)
                    printf("Matching %c(cur_char) to %c(oprand)\n", cur_char, 
                cur_action.oprand);
                if(cur_char == cur_action.oprand)
                {
                    if((flags & DEBUG) == DEBUG)
                        printf("Adding to next_list\n");
