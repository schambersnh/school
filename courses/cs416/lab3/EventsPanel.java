                         ...
                         implements Animated, MouseListener, MouseMotionListener
{
   ...
   public EventsPanel( Container parent ) 
   {
      ...
      JPlayer myJPlayer2 = new JPlayer( Color.RED, 30, 30);
      myJPlayer2.setMove(10, 10);
      this.add(myJPlayer2);
   ...
   {  
     ...
     _dragging = findSelectedDraggers(_draggers, _saveMouse);
     if(_dragging.size() > 0)
     {
   ...
   { 
     ...
     {
      Point temp = me.getPoint();
      int currentX = temp.x;
      int currentY = temp.y;
      
      int diffX = currentX - _saveLoc.x;
      int diffY = currentY - _saveLoc.y;
      
      for(int i = 0; i < _dragging.size(); i++)
      {
        _dragging.get(i).moveBy(diffX, diffY);
      }
      _saveLoc = temp;
     }
