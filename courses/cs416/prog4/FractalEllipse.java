...
public class FractalEllipse extends Ellipse2D.Double 
{
   ...
   public FractalEllipse( int depth, Point p, int w, int h )
   {
      ...
      if(depth < maxDepth)
      {
        ...
        for(int i = 0; i < numChildren; i++)
        {
          ...
          x = radius  *  Math.cos(_angle + (Math.PI/2*offset) ) + centerx;
          y = radius  *  Math.sin(_angle + (Math.PI/2*offset)) + centery;
          
          x -= w/4;
          y -= h/4;
          FractalEllipse temp = new FractalEllipse( depth + 1, new Point((int)x, (int)y), w/2, h/2);
          temp.setFrame(temp.getX(), temp.getY(), getWidth()*sizeRatio, getHeight()*sizeRatio);
          _children.add(temp); 
        }
   ...
   myColors[4] = new Color(67, 190, 84, 250); 
