 ...
 *    the second?
 * 
 * Stacks are a last in first out structure, which means that adding elements
 * and removing elements from a stack from the START of a vector would be 
 * counter-intuitive. Adding and removing from the end is the most efficient way
 * 
 * 
 */
...
public class Stack<T>
{
   ...
   public T pop()
   {
      ...
      if ( _stack.size() > 0 )
      {
         retVal = _stack.get( 0 ); //remove from beginning
         _stack.remove( 0 );
      }
      return retVal;
   }
   ...
   {
      _stack.add(item );  // add to end of Vector
   }
   ...
   public T pop2()
   {
      ...
      if ( _stack.size() > 0 )
      {
         retVal = _stack.get( _stack.size() - 1 ); //remove from end
