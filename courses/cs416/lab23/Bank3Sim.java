 ...
 * Bank3Sim
 * ---------
 * Step 4g  wait( 1000 )   4.184             0.0235
 *          wait( 500 )    3.665             0.019399999
 *          wait( 100 )    3.672             0.1017
 * 
 * Step 4h  interrupt      3.305             0.0251
 **************************************************************************
 */
import java.util.*;
import java.awt.*;
...
public class Bank3Sim
{
  ...
  { 
    ...
    Teller[] myTellers = new Teller[3];
    for(int i = 0; i < myTellers.length; i++)
    {
      myTellers[i] = new Teller( "T" + i, clientsIn, clientsOut );
      myTellers[i].start();
    ...
    try
    {
      for(int i = 0; i < myTellers.length; i++)
      {
        myTellers[i].interrupt();
        myTellers[i].terminate();
      }
      for(int j = 0; j < myTellers.length; j++)
      {
        myTellers[j].join();
      }
    }
  ...
  public static void main( String[] args )
  {
    ...
    int serviceRate = Utilities.getArg( args, 1, 200 );
    int maxClients  = Utilities.getArg( args, 2, 30 );
