...
public class Unifier {
	 ...
	 */
	public ArrayList<Clause> unifyClauses(Clause c1, Clause c2){
		...
		for(Pair<Integer,Integer> pair: pairList){
			...
			if(unifiable){
				copy1.removePredicate(pair.getLeft());
				copy2.removePredicate(pair.getRight());
				Clause temp = new Clause();
				for(Predicate p1: copy1.getPredicates()){
					temp.addPredicate(p1);
				}
				for(Predicate p2: copy2.getPredicates()){
					temp.addPredicate(p2);
				}
				retval.add(temp);
			}
		}
